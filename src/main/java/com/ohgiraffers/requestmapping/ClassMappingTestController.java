...
* 클래스 레벨에 URL을 공통 부분ㄴ을 이용해 설정하고 나면 매번 핸들러 메소드의 URL에 중복 되는 내용을 작성하지 않아도 된다.
* 와일드 카드를 이용해서 조금 더 포괄적인 URL 패턴을 설정할 수 있다. */
@Controller
@RequestMapping("/order/*") // 와일드 카드 * <- 모든
public class ClassMappingTestController {
    ...
    * */
    ...
    public String selectOrderDetail(Model model, @PathVariable("orderNo") int orderNo) { // 변수명이 일치하지 않으면 어노테이션 뒤에("")를 일치시켜야 코드가 읽힌다.

        /* parsing 불가능한 PathVariable이 전달 되면 400 (Bad Request) 에러가 발생한다.
    ...
    /* 4. 그 외의 다른 요청 */
    /* @RequestMapping 어노테이션에 아무런 URL을 설정하지 않으면 요청 처리에 대한 핸들러 메소드가 준비되지 않았을 때
    * 해당 메소드를 호출한다. */
    @RequestMapping
